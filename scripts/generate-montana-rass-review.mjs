...
const submissionDocs = [
  ...
  ["mt-alcohol-full-course-content.pdf", "On-Premises Full Course Content", "Readable full course export for the on-premises track."],
  ["mt-alcohol-offsale-full-course-content.pdf", "Off-Premises Full Course Content", "Readable full course export for the off-premises track."],
];
...
function renderSidebar(track, modules, activeId = "exam") {
  ...
  return `<aside class="course-sidebar">
    ...
    <nav class="module-list" aria-label="${esc(track.label)} modules">
      ...
      <a class="${activeId === "exam" ? "active" : ""}" href="exam.html"><span>✓</span><strong>Final Exam Preview</strong><small>${track.examDraw} of ${track.pool}</small></a>
    </nav>
  </aside>`;
}
...
function buildModulePage(track, config, modules, mod, index) {
  ...
  fs.writeFileSync(path.join(outDir, "courses", track.slug, `${mod.moduleId}.html`), page({ title: `${track.label} Module ${mod.moduleId}: ${value(mod.title)}`, body, depth: 2, pageClass: "course-page" }));
}
...
function buildExamPage(track, modules) {
  ...
  const body = `<main class="course-layout">
    ...
    <article class="course-main">
      ...
      <section class="module-hero exam-hero">
        <div class="module-meta"><span>${esc(track.examDraw)} questions per attempt</span><span>${esc(track.pool)} question pool</span><span>80% to pass</span></div>
        <h1>${esc(value(exam.title))}</h1>
        <p>${esc(value(exam.instructions))}</p>
      </section>
      <section class="learn-card">
        <h2>Reviewer note</h2>
        <p>This static preview shows the full pool and answer key so Montana CARD can evaluate coverage. The live LMS draws ${esc(track.examDraw)} questions per attempt, shuffles questions and options, and applies the ARM 42.13.907 retake rule.</p>
      </section>
  ...
  fs.writeFileSync(path.join(outDir, "courses", track.slug, "exam.html"), page({ title: `${track.label} Final Exam Preview`, body, depth: 2, pageClass: "course-page" }));
}
...
function buildTrackHome(track, config, modules) {
  return `<section class="track-card" id="${esc(track.slug)}">
    ...
    <div class="track-stats">
      ...
      ${stat("Final exam", `${track.examDraw} of ${track.pool}`)}
      ${stat("Modules", `${modules.length}`)}
    </div>
    <div class="track-actions">
      <a class="primary" href="${moduleHref(track.slug, modules[0].moduleId)}">Click Through Course</a>
      <a href="courses/${track.slug}/exam.html">Open Final Exam Preview</a>
      <a href="pdf/${esc(track.pdf)}">Open Course PDF</a>
    </div>
...
function buildIndex(trackData) {
  const body = `<main>
    <section class="review-hero">
      ...
      <div class="hero-panel">
        ...
        <ul>
          ...
          <li>Video and podcast overview players hosted locally</li>
          <li>Infographics, scenarios, quiz questions, and answer keys</li>
          <li>Final-exam pool previews and core PDFs</li>
        </ul>
      </div>
    </section>
    ...
    <section id="review-course" class="review-section">
      <div class="section-copy">
        ...
        <p>Open either track and move module to module with the same basic flow as the CertReady learner preview: course sidebar, module overview media, lesson sections, embedded infographics, quizzes, and final exam preview.</p>
      </div>
  ...
  fs.writeFileSync(path.join(outDir, "index.html"), page({ title: "CertReady Montana RASS Reviewer Mirror", body }));
}
...
function writeCompatibilityRedirects(trackData) {
  fs.writeFileSync(path.join(outDir, "on-premises.html"), redirectPage("On-Premises Preview", "courses/mt-alcohol/01.html"));
  fs.writeFileSync(path.join(outDir, "off-premises.html"), redirectPage("Off-Premises Preview", "courses/mt-alcohol-offsale/01.html"));
  fs.writeFileSync(path.join(outDir, "exam-on-premises.html"), redirectPage("On-Premises Exam Preview", "courses/mt-alcohol/exam.html"));
  fs.writeFileSync(path.join(outDir, "exam-off-premises.html"), redirectPage("Off-Premises Exam Preview", "courses/mt-alcohol-offsale/exam.html"));
